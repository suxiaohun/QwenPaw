...
async def test_scenario_a_retries_once_then_raises(
    ...
    monkeypatch.setattr(
        "qwenpaw.agents.tools.websearch.asyncio.sleep",
...
async def test_scenario_b_parses_credentials_stores_and_retries(
    ...
    monkeypatch.setattr(
        "qwenpaw.agents.tools.websearch.get_current_workspace_dir",
        lambda: tmp_path,
    )

    put_mock = AsyncMock()
    with patch(
        "qwenpaw.agents.tools.websearch.AsyncCredentialStore.put",
        put_mock,
    ):
        provider = AnySearchProvider()
